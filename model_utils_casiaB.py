...
from . import config
from .my_models_casiaB import CenterLossLayer
...
def read_rnn_model(angle):
    ...
    json_string = open(config.casiaB_rnn_model_path).read()
    model = model_from_json(json_string, 
                    custom_objects={'CenterLossLayer': CenterLossLayer})

    rnn_model_weight_path = os.path.join(config.model_dir,
                            angle + "_" + config.casiaB_rnn_model_weight)
    
    model.load_weights(rnn_model_weight_path)
    
    print("loaded model directory: ", angle + "_" + config.casiaB_rnn_model_weight)
    return model
