...
def make_data_for_encoder(subject_id_list,
                                  walking_seq,
                                  data_type,
                                  start_id,
                                  angle_list,
    ...
    for s_id, subject_id in enumerate(subject_id_list):
        ...
        # reordering list
        angle_list.remove(transform_angle)
        angle_list =  [transform_angle] + angle_list
        root_seq_ts = []
        ...
        for a, angle in enumerate(angle_list):
            ...
            for ws, seq in enumerate(walking_seq):
                first_frame_bkps = []
                ...
                seq_data, seq_label = get_format_data(subject_id, seq_kps, seq,  start_id)
                
                if (a == 0):
                    root_seq_ts.append(seq_data.shape[0])

                elif(a > 0):
                    if (seq_data.shape[0] > root_seq_ts[ws]):
                        seq_data = seq_data[:root_seq_ts[ws]]

                    elif(seq_data.shape[0] < root_seq_ts[ws]):

                        dummy_ts = np.zeros((root_seq_ts[ws] - seq_data.shape[0],
                        config.casiaB_nb_steps, config.casiaB_nb_features), dtype = np.float32)

                        seq_data = np.vstack((dummy_ts, seq_data))

                print(seq, " total timestep: ", seq_data.shape[0])
            ...
            if(a == 0):
                sub_data = angle_data
                sub_label = angle_data
                root_label = angle_data
            else:
                sub_data  = np.vstack((sub_data, angle_data))
                sub_label = np.vstack((sub_label, root_label))


        print("\nsub data shape:", sub_data.shape)
        print("sub label shape:", sub_label.shape)

        # collecting all subject data
        if(s_id == 0):
            total_dataset = sub_data
            total_dataset_label = sub_label
        
        else:
            total_dataset = np.vstack((total_dataset, sub_data))
            total_dataset_label = np.vstack((total_dataset_label, sub_label))
...
def load_encoder_train_data_per_group(data_type):
    ...
    if(data_type == "train"):
        data, label  = make_data_for_encoder(
                                        total_id_list [start_id-1:end_id],
                                        config.casiaB_ls_gallery_train_seq,
                                        data_type,
                                        start_id,
                                        angle_group, transform_angle)
...
if __name__ == "__main__":
    load_encoder_train_data_per_group("valid")